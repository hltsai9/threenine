...
| **👤 name tag** (left edge) | The **handover route**: `Mia → Ren` = previous-shift operator → who has it on the current shift; plain `Mia` = picked by a current-shift operator; a highlighted **`Ren → ?`** = the shift changed and **no one on the current shift has taken the case** — act on these first |
