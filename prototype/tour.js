...
(function () {
  ...
  const STEPS = [
    ...
    },
    {
      kind: 'pointer',
      route: '#/queue',
      selector: '.queue-empty, .queue-card',
      placement: 'bottom',
      title: 'Action Queue — your curated to-do list',
      body: 'The queue is empty by default. You decide which cases to track by clicking + Queue on the Cases page (or from a case detail). The queue is just the cases you have personally picked to keep an eye on — like a starred inbox.',
    },
    {
      kind: 'pointer',
      route: '#/cases',
      selector: '.kanban',
      placement: 'bottom',
      title: 'Cases — kanban by status',
      body: 'Active cases for the current week are grouped into four columns by status: New → With Local FIT → With HQ Product Team → Sanity Check / With Requester. You see at a glance who has the watch on each case. Closed and cancelled cases drop off the board.',
    },
    {
      kind: 'pointer',
      route: '#/cases',
      ...
      placement: 'right',
      title: '+ Queue to add a case',
      body: 'Each kanban card has a small + Queue button. Click it to pin the case to your Action Queue; click again (now ✓ Queued) to remove it. Clicking the card body itself opens the reading panel below.',
    },
    {
      kind: 'pointer',
      route: '#/cases',
      ...
      placement: 'top',
      title: 'Reading panel',
      body: 'Click any kanban card to populate this panel with the case\'s routing, two clocks, latest handover note, notes, and recent history — no navigation needed. Use Open full case → for the full editable detail page.',
    },
    {
      kind: 'pointer',
      route: '#/cases/C-1044',
