...
// the hand-off time. The modal pre-fills the standard rule's deadline (in the active display
// zone) and stores whatever they confirm as `trackStatusDueAt`. `selectEl` is reverted to the
// committed value so the dropdown never shows an uncommitted choice if they cancel.
function openHandoffTimeModal(c, value, prev, selectEl) {
  const def = TRACK_STATUS_BY_ID[value];
  // Default = the standard rule's slot, anchored at now (computed on a probe so nothing commits yet).
  const probe = { ...c, trackStatus: value, trackStatusAt: NOW.toISOString(), trackStatusDueAt: null };
  const h = scheduledHandoff(probe);
  const defMs = h ? Date.parse(h.dueAt) : NOW.getTime() + 24 * 3600 * 1000;
  const inputVal = isoToLocalInput(new Date(defMs).toISOString());
...
function _renderWatchRow(c, top) {
  ...
  const idEl = `<div class="rb-watch-id" style="left:${idShift};">${escapeHtml(c.id)} · ${itTimeLabel(c)}</div>`;
