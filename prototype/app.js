...
// <option>s for the members of a Core Team desk; `selectedId` pre-selects one. Leads with a
// "no specific member" choice so a desk can be assigned without naming a person.
function coreMemberOptions(deskId, selectedId) {
  ...
  return `<option value="">— No specific member —</option>` + members.map(m =>
...
function openCorePreassignModal(c, op) {
  ...
  `, (modal) => {
    ...
    c.coreId = coreId;
    c.coreMemberId = fieldVal(modal, 'coreMemberId') || null;
...
function handleReassign(caseId, type) {
  ...
  showModal(`
    ...
    ${type === 'core' ? `<label>Core Team member</label>
