...
// needsHandoverNote is still true (no fresh note authored this shift). Sanity Check cases are
// excluded (they park in their own group and aren't part of the shift hand-off). Not gated on
// shift-ending time: it's a running reminder of what's left to hand over.
function myHandoverPendingCases() {
  const me = STATE.operatorId;
  return pickedCases().filter(c =>
    caseTrackerOperatorId(c) === me && needsHandoverNote(c) && !isSanityCase(c));
}
...
function caseHandoverNotes(c) {
  ...
  for (const h of (c.history || [])) {
    if (h.kind !== 'handover' || h.deleted) continue;   // soft-deleted notes are hidden here
...
// Delete ONE handover note (identified by its history timestamp) from a case. SOFT delete —
// the history entry (and its text) is kept as an audit record, just flagged `deleted` so every
// read path (panel, export, shift activity, history list) hides it; the text stays queryable in
// the stored payload / DB. The UI handler adds the confirm dialog / audit marker around it.
// When the deleted note was the CURRENT c.handover, the newest remaining (non-deleted) note is
// promoted back into c.handover (from → to parsed back out of its history detail; marked stale
// so the shift-handover discipline still asks for a fresh note); with no notes left, c.handover
// clears. `byId` (optional) records who deleted it. Returns true when it deleted.
function deleteHandoverNote(c, at, byId) {
  const entry = (c.history || []).find(h => h.kind === 'handover' && h.at === at && !h.deleted);
  const wasCurrent = !!(c.handover && c.handover.at === at);
  if (!entry && !wasCurrent) return false;
  if (entry) {
    entry.deleted = true;
    entry.deletedAt = new Date(NOW).toISOString();
    if (byId) entry.deletedBy = byId;
  }
  if (wasCurrent) {
    // Clear FIRST — caseHandoverNotes has a fallback that re-adds c.handover when no matching
    // (non-deleted) history entry exists, which would resurrect the note we just hid.
    c.handover = null;
    ...
    if (!last) {
      ...
      const h = (c.history || []).find(x => x.kind === 'handover' && x.at === last.at && !x.deleted);
...
function renderCaseList() {
  ...
  const handoverBanner = myPending.length > 0 ? `
    <div class="handover-reminder">
      ...
      <span class="handover-reminder-cases">${myPending.map(c => `<a class="mono handover-reminder-case" data-action="reminder-goto-case" data-case-id="${c.id}" href="#/cases/${c.id}" title="${escapeHtml(c.subject)}">${c.id}</a>`).join(' ')}</span>
    </div>
  ` : '';
...
function renderCaseDetailBody(c) {
  ...
  const allNotes = caseHandoverNotes(c);
  ...
  const history = (c.history || []).filter(h => !h.deleted).slice().reverse().map(h => {
    const op = getOperator(h.who);
    const who = op ? op.name : h.who;
    return `
    <li>
    ...
    <div class="detail-grid">
      <div>
        <div class="card"><div class="card-body">
          ...
          <div class="detail-section">
            <h3>Handover notes${allNotes.length ? ` <span class="muted tiny" style="font-weight:400">· ${allNotes.length}</span>` : ''}</h3>
            ${allHandoversHtml}
          </div>
          <div class="detail-section">
            <h3>Notes</h3>
...
function renderShiftDetail(shiftName) {
  ...
  for (const c of allOpen) {
    for (const h of (c.history || [])) {
      if (h.kind === 'handover' && !h.deleted && sh.operatorIds.includes(h.who)) {
...
function wireHandoverCombo() {
  ...
  const combo = modal && modal.querySelector('[data-combo]');
  if (!combo) return;
  ...
  const applyFilter = () => {
    ...
    items.forEach(li => {
      ...
      li.hidden = !match;
      if (match) shown++;
      li.classList.toggle('combo-item-sel', !!hidden.value && li.dataset.id === hidden.value);
    });
    combo.querySelector('[data-combo-empty]')?.remove();
    if (!shown) {
...
function handlePrompt(caseId, kind) {
  ...
  if (kind === 'end_of_shift_handover') {
    ...
    // Custom filter dropdown (not a native <datalist>): typing narrows the list to matching
    // names, and an "×" clears the keyword filter. A hidden field holds the chosen operator id.
    const recipientField = recipients.length ? `
      <label>Hand over to * <span class="muted tiny">— type to filter</span></label>
      <div class="combo" data-combo>
        <div class="combo-input-wrap">
          <input type="text" class="combo-input" data-field="recipient" autocomplete="off"
                 value="${escapeHtml(existingRecipient ? rLabel(existingRecipient) : '')}"
                 placeholder="Type a teammate's name">
          <button type="button" class="combo-clear" data-combo-clear aria-label="Clear filter" title="Clear">×</button>
        </div>
        <input type="hidden" data-field="recipientId" value="${escapeHtml(existingRecipient ? existingRecipient.id : '')}">
        <ul class="combo-list" data-combo-list>
          ${recipients.map(o => `<li class="combo-item" data-id="${escapeHtml(o.id)}" data-label="${escapeHtml(rLabel(o))}">${escapeHtml(rLabel(o))}</li>`).join('')}
        </ul>
      </div>
    ` : '';
    showModal(`
    ...
    `, (modal) => {
      ...
      // roster falls back to the opposite shift). Prefer the clicked id; else resolve the typed
      // text to an operator by its "Name · Shift" label, or just the name.
      let recipient = null;
      if (recipients.length) {
        const chosenId = fieldVal(modal, 'recipientId').trim();
        const typed = fieldVal(modal, 'recipient').trim();
        if (chosenId) recipient = recipients.find(o => o.id === chosenId) || null;
        if (!recipient && typed) {
          const t = typed.toLowerCase();
          recipient = recipients.find(o => rLabel(o).toLowerCase() === t)
            || recipients.find(o => o.name.toLowerCase() === t);
        }
        if (!recipient) return modalError('Choose who to hand this over to.');
      }
...
// Accept either a raw case ID or a full case link and return the case ID. From a link the id is
// the last "/"-separated path segment (query string / fragment stripped first); Case Center ids
// start with "Case-", so a segment matching that is preferred over a trailing segment when both
// are present. A plain id (no slash) is returned as-is.
function extractCaseId(input) {
  let s = String(input || '').trim();
  if (!s) return '';
  s = s.split(/[?#]/)[0].trim();          // drop ?query and #fragment
  if (s.includes('/')) {
    const segs = s.split('/').filter(Boolean);
    const caseSeg = segs.filter(x => /^case-/i.test(x)).pop();   // prefer a "Case-…" segment
    s = caseSeg || (segs.length ? segs[segs.length - 1] : s);
  }
  return s.trim();
}
...
function handleNewCase(op) {
  ...
  showModal(`
    ...
    <div class="modal-sub">Pulls this case from Case Center and adds it to the board. Paste the case ID or the full case link.</div>
    <label>Case ID or link *</label>
    <input type="text" data-field="caseId" placeholder="e.g. Case-581234 or https://…/Case-581234">
    <div class="modal-actions">
      <button class="btn" data-modal-cancel>Cancel</button>
      <button class="btn btn-primary" data-modal-submit>Fetch &amp; add</button>
    </div>
  `, (modal) => {
    const raw = fieldVal(modal, 'caseId').trim();
    if (!raw) return modalError('Enter a case ID or paste the case link.');
    const id = extractCaseId(raw);
    if (!id) return modalError('Could not read a case ID from that — check the link or ID.');
...
function bindHandlers() {
  ...
  // Reminder-banner case links: select the case in the Route Board / reading panel in place
  // (no navigation away) so the operator can read it and write the note without leaving. Falls
  // back to the full detail route if the case somehow isn't in the picked workspace.
  document.querySelectorAll('[data-action="reminder-goto-case"]').forEach(el => {
    el.addEventListener('click', e => {
      e.preventDefault();
      const id = el.dataset.caseId;
      const c = caseById(id);
      if (c && isPicked(c)) {
        STATE.kanbanSelected = id;
        STATE.pickedListTab = isSanityCase(c) ? 'sanity' : 'cases';
        render();
        const row = document.querySelector(`.rb-row[data-case-id="${id}"], .picked-row[data-case-id="${id}"]`);
  ...
  document.querySelectorAll('[data-action="delete-handover-note"]').forEach(el => {
    el.addEventListener('click', e => {
      ...
      `, () => {
        const op = getOperator(STATE.operatorId);
        if (!deleteHandoverNote(c, at, op ? op.id : STATE.operatorId)) return true;
